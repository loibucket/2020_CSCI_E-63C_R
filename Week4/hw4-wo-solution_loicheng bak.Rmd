...
#untransformed plot
dfexp = df2plot
dfexp$err = exp(df2plot$err)
avgexperr = exp( sqrt(mean((df[,"contrib"]-mean(df[,"contrib"]))^2)) )
plot(dfexp,xlab="Number of variables",ylab="Regression error (untransformed)",main=paste(nrow(df),"observations"),ylim=c(min(dfexp[,"err"]), avgexperr))
abline(h=avgexperr,lty=2)
...
#untransformed plot
plot(dfexp,xlab="Number of variables",ylab="Regression error (untransformed)",main=paste(nrow(df),"observations (No avg error line)"))
abline(h=avgexperr,lty=2)
...
**Unlike the dample data, the most dramatic reduction here occurs after 4 predictors are included in the model.  The next most dramatic reduction occurs after adding 6 predictors.   The reduction in error is much flatter compared with the sample data.  The difference between 7 and 78 predictors is about 0.7%, while for the sample data shows about 7% reduction in error going from 3 to 27 predictors.**
...
```{r}
bootErrRes <- bootTrainTestErrOneAllVars(dfY,30)
```
...
plotBootRegrErrRes(bootErrRes,main="Regression Error (Log Transformed)",ylim=c(min(colMeans(bootErrRes$bootTrain)),sqrt(mean((mean(dfY[,"Y"])-dfY[,"Y"])^2))))
abline(h=sqrt(mean((mean(dfY[,"Y"])-dfY[,"Y"])^2)),lty=2)

#plot untransformed
expErr = bootErrRes
expErr$bootTrain = exp(bootErrRes$bootTrain)
expErr$bootTest = exp(bootErrRes$bootTest)
expErr$allTrain = exp(bootErrRes$allTrain)
expAvgErr = exp(sqrt(mean((mean(dfY[,"Y"])-dfY[,"Y"])^2)))

plotBootRegrErrRes(expErr,main="Regression Error (Un Transformed)",ylim=c(min(colMeans(expErr$bootTrain)),expAvgErr))
abline(h=expAvgErr,lty=2)

#####repeat, without avg line ###

plotBootRegrErrRes(bootErrRes,main="Regression Error (Log Transformed, No avg error line)")
abline(h=sqrt(mean((mean(dfY[,"Y"])-dfY[,"Y"])^2)),lty=2)

plotBootRegrErrRes(expErr,main="Regression Error (Untransformed, No avg error line)")
abline(h=expAvgErr,lty=2)
...
compete_err = sqrt(mean((df_compete$contrib - df_compete$predcontr)^2))
compete_err
